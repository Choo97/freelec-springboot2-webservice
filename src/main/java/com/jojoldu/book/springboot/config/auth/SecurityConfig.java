...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()
                .headers().frameOptions().disable() //h2-console을 위한 위 옵션들 비활성화
                .and()
                .authorizeRequests() // URL 또는 HTTP 메서드별로 관한관리를 해주기 위한 시작점, 그래야 antMatchers를 사용 가능
                .antMatchers("/", "/css/**", "/images/**",
                        "/js/**", "/h2-console/**").permitAll() // 위에 해당하는 URL들은 전체 권한 부여
                ...
                .and()
                .oauth2Login() // auth2 로그인 기능에 대한 설정 진입점
